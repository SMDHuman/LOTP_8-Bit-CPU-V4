...
def goto(adrs):
    adrs = max(min(2**addressSize-1, adrs), 0)
    adrs = int2binlist(adrs, addressSize)
...
RB       = Pin(28, Pin.OUT, value = 0)
A12, WE  = Pin(27, Pin.OUT, value = 0),  Pin(0, Pin.OUT, value = 1)
A7,  DU  = Pin(26, Pin.OUT, value = 0),  Pin(1, Pin.OUT, value = 0)
...
addressPins = [A0, A1, A2, A3, A4, A5, A6, A7, A8, A9, A10, A11, A12]
...
address = 0
addressSize = len(addressPins)


file = open("OutputDisplayROM", "r")
for line in file.readlines()[1:]:
    ...
    for data in line:
        ...
        progress = address*100/(2**addressSize-1)
        if(round(progress) == round(progress, 1)):
            print(f"{address}/{2**addressSize} - %{int(progress)}")
